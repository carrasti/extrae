...
class Model extends Backbone.Model
    ...
    extractData: (scraper, root, options) ->
        ...
        for fieldKey, fieldItem of ThisClass.fieldDefinitions

            # if there is a extractrule for the field key apply the rule
            if ThisClass.extractRules[fieldKey]?
                @set fieldKey, (ThisClass.extractRules[fieldKey].process root, options, @)
